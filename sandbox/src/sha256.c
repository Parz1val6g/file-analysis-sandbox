/* SHA-256 — FIPS 180-4 compliant, zero external dependencies.
 *
 * Implementation uses only uint32_t bitwise operations.
...
void sha256_update(Sha256Ctx *ctx, const uint8_t *data, size_t len) {
    ...
    for (i = 0; i < len; i++) {
        ...
        if (ctx->blocklen == SHA256_BLOCK_SIZE) {
...
void sha256_final(Sha256Ctx *ctx, uint8_t hash[SHA256_DIGEST_SIZE]) {
    ...
    if (ctx->blocklen > 56) {
        while (ctx->blocklen < SHA256_BLOCK_SIZE) ctx->block[ctx->blocklen++] = 0;
        sha256_transform(ctx, ctx->block);
        ctx->blocklen = 0;
    }
    while (ctx->blocklen < 56) ctx->block[ctx->blocklen++] = 0;
    for (i = 0; i < 8; i++) ctx->block[56 + i] = (uint8_t)(ctx->bitlen >> (56 - i*8));
    sha256_transform(ctx, ctx->block);
